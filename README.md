...
- **ロックの範囲**
  - トランザクション内でレコードを取得すると、レコードにロックがかかる
  - ただし、これは他のトランザクションによる「更新」をブロックするのみで、「読み取り」は制限されない

- **同時アクセス時の挙動**
  - 複数のトランザクションが同じレコードを同時に取得可能
...
Firestoreでの順序処理の実現方法
Firestoreで順序処理を実現するには、自前でキューイングの仕組みを実装するのが効果的。専用のキューイング用コレクションを用意し、定期的にポーリングすることで、自分の順番が来るのを待つ形で順序制御を行う。

このプロジェクトでの具体的な実装
使用するコレクション
`que`コレクションを作成し、トランザクションを順序管理。
...
### 注意点
- ポーリングによるFirestoreへのリクエスト数を削減するために、リトライ間隔やタイムアウト値のチューニングが不可欠
- キューの整合性が担保されるよう、障害発生時の対応シナリオを含めて実装を設計する必要がある
