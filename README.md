...
結論から言うと**Firestoreの悲観ロックは本質的には楽観ロックの挙動をする**。トランザクションを開始した後にレコードを取得するとレコードに対してロックがかかるが、これは読み取りを制限するものではなく、**他のトランザクションによる更新をブロックするのみ**にとどまる。
これはfirstoreがスケーラビリティを担保するための設計思想ではあるが、不特定多数のユーザーが殺到した時に順番にチケット購入処理を行うような
 - 1.購入対象のチケットを取得し、読み取り制限をかける
 - 2.購入対象のチケットを更新する(他のトランザクションによって更新されている可能性がないため、確実に更新できる)

のような処理は行うことができない。
1の時点で**他のトランザクションも同じチケットを取得している可能性がある**ため、2の時点で更新を行うとするとすでに他のトランザクションによって更新済であることが起きてしまう。
その結果、**2で更新が失敗しトランザクションを何度もリトライする挙動**となる。

更新の競合の発生頻度が高くない場合はトランザクションのリトライによって成功するのを待つ設計にするのもシンプルで悪くなく、問題は起きにくい。
急にサービスが成長するなどして、**大勢のユーザーが一挙に同じドキュメントを更新するような状況になった時に、急にタイムアウトが発生**するようになる可能性がある。
そのような場合は、**自前のキューイング処理**などを実装する必要がある。
